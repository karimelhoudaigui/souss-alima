  ...
  return (
    <article>
      <header className="container-page border-b border-line py-8 md:py-12">
        <Link className="ui-sans text-sm font-medium text-muted hover:text-ink" href="/articles">← Tous les articles</Link>
        <p className="page-kicker mt-8">{theme}</p>
        {article.titleAr ? <p className="arabic-title mt-4" dir="rtl" lang="ar">{article.titleAr}</p> : null}
        <h1 className="mt-3 max-w-4xl text-4xl font-medium text-ink md:text-6xl">{article.title}</h1>
        <p className="page-description">{article.summary}</p>
        <div className="ui-sans mt-5 flex flex-wrap items-center gap-4 text-sm text-muted">
